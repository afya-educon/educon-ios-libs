 ...
 */
@objc public protocol MXScrollViewDelegate: UIScrollViewDelegate {
    /**
     ...
     */
    func scrollViewShouldScroll(_ scrollView: MXScrollView, with subView: UIScrollView) -> Bool
}

/**
 The MXScrollView is a UIScrollView subclass with the ability to hook the vertical scroll from its subviews.
 */
open class MXScrollView : UIScrollView {
    static var KVOContext = "kMXScrollViewKVOContext"

    /**
     Delegate instance that adopt the MXScrollViewDelegate.
     */
    var forwarder: MXScrollViewDelegateForwarder!
    ...
    override open var delegate: UIScrollViewDelegate? {
        ...
        set {
            forwarder.delegate = newValue as? MXScrollViewDelegate
            super.delegate = nil
    ...
    func initialize() {
        forwarder = MXScrollViewDelegateForwarder(scrollView: self)
        super.delegate = forwarder
        ...
        addObserver(self, forKeyPath: #keyPath(UIScrollView.contentOffset),
                    options:[.new, .old], context: &MXScrollView.KVOContext)
        isObserving = true
    }

    deinit {
        removeObserver(self, forKeyPath: #keyPath(contentOffset), context: &MXScrollView.KVOContext)
        removeObservedViews()
    }
}

extension MXScrollView: UIGestureRecognizerDelegate {
...
extension MXScrollView {
    ...
    func addObserver(to scrollView: UIScrollView) {
        ...
        scrollView.addObserver(self,
                               ...
                               options: [.old, .new],
                               context: &MXScrollView.KVOContext)
    }
    ...
    override open func observeValue(forKeyPath keyPath: String?,
        ...
        guard context == &MXScrollView.KVOContext && keyPath == #keyPath(UIScrollView.contentOffset) else {
...
extension MXScrollView: UIScrollViewDelegate {
